...
10. In the web browser, make a note of the URL of the web API service. It will have the form `https://backendnnnnnnnn.azurewebsites.net`, where `nnnnnnnn` is a string of digits intended to give each web app a unique URL. You will require this URL later when you deploy the **FrontEnd** web application.
...
7. Open the **appsettings.json** file in the root folder of the **BackEnd** project. Replace the value of the **DefaultConnection** property to the string returned in the previous step. In this string, replace *\<username\>* with **azureadmin**, and replace *\<password\>* with the password that you specified when you created the Azure SQL Server. The file should look similar to this:
...
8. Move to the root folder of the **BackEnd** project.
...
13. Create an Azure Web App using the App Service Plan. Wach Azure App service must have a unique name, so replace *\<service-name-suffix\*> with your initials and the current data in numeric format. For example **JPWS01012020**.
...
16. In the web browser, make a note of the URL of the web API service. It will have the form `https://backend<service-name-suffix>.azurewebsites.net`. You will require this URL later when you deploy the **FrontEnd** web application.
...
## Prepare the Front project for migration
...
6. Open the **IdentityHostingStartup.cs** file in the **Areas\Identity** folder of the **FrontEnd** project.

7. In the **Configure** method, amend the `services.AddDbContext` statement. Replace the call to the **UseSqlite** method with **UseSqlServer**, as shown below:
...
3. Open the **IdentityHostingStartup.cs** file in the **Areas\Identity** folder of the **FrontEnd** project.
...
# Publish the BackEnd web API service and FrontEnd web application as Azure Linux Web Apps
