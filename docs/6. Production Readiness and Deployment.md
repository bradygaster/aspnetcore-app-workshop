...
- **If you're using Visual Studio 2019:**
    ...
    3. On the **Debug** menu, click **Start Debugging**.
        ...
        When the Docker container has been started, a web browser window will open at the URL for the **BackEnd** web API service, and navigate to the **api/speakers** endpoint. You may be prompted to download the file **speakers.json**. This file will contain an empty list of speakers. The image below shows the dialog box displayed by Internet Explorer. If you are using a different browser, you may see a different message.

        ![The *View and track downloads** dialog box for Internet Explorer.](images/6-download.png)

        Close the dialog box without downloading the data.

    4. In Visual Studio, select the **Containers** window, and click the **Containers** and **Ports** tabs. In the left-hand pane listing containers, select the **BackEnd** container.

        ![The *Containers** window in Visual Studio. The user is displaying the ports for the **BAckEnd** container.](images/6-containers.png)

    5. In the list of ports for the **BackEnd** container, click the **Host Port** link for **Container Port** 443 (port 32796 in the image above). The web browser will open a page at this port running on the server in the container. This is the URL for the **BackEnd** web API service in the container. The web browser will display an HTTP 404 error message, for reasons described in earlier exercises.

    6. In the web browser, move to the Swagger endpoint for the **BackEnd** service, `https://localhost:<port>/swagger`. The Swagger UI page should appear.

    7. In the **Sessions** section, click the **POST** button for the **/api/Sessions/upload** operation, and then click **Try it out**.

    8. In the **Parameters** section, at the file prompt, click **Browse**.

    9. In the Open dialog box, browse to the folder containing your clone of the GitHub repository, move to the folder **src/BackEnd/Data/Import** in the repository, select the file **NDC_London_2020.json**, and then click **Open**.

    10. Back in the web browser, click **Execute**, and wait for the data to be uploaded. Verify that the upload is successful (the server response code should be 200).

    11. Return to the **Containers** window in Visual Studio.

    12. In the left-hand pane, select the **FrontEnd** container.

    13. In the list of ports click the **Host Port** link for **Container Port** 443. The web browser will open the home page at for the **FrontEnd** application, displaying the list of sessions for the conference.

    14. Verify that you can register, sign in, and create a personal agenda.

    15. When you have finished, in Visual Studio, on the **Debug** menu, click **Stop Debugging**.
...
# Deploy the BackEnd web API service to an Azure App Service using Azure DevOps

In this exercise, you'll deploy the **BackEnd** web API service to an Azure App Service. You'll use Azure DevOps to manage the deployment process. Azure DevOps supports CI/CD scenarios, enabling you to quickly deploy updates the the web API service as the code is modified and released after testing.
