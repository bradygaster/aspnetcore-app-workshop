...
- A Linux App Service, also running in Azure, using GitHub deployment.
...
# Publish the BackEnd web API service as an Azure Linux Web App using GitHub deployment

In this exercise, you'll deploy the **BackEnd** web API service Azure, running as a Linux Web App (you'll leave the **FrontEnd** web app running locally, to save time and avoid repetition). This task assumes you have an Azure subscription. If you don't already have an Azure account, you can sign up for a free account on the [Azure page of the Microsoft website](https://azure.microsoft.com/free/). As a variatioon on the Windows Web App deployment scenario, you'll use GitHub deployment in this exercise. GitHub deployment enables you to publish updates to the web apps more easily, simply by pushing the changes to a GitHub repository managed by the Azure App service.
...
2. If you have been using SQL Server, switch the **BackEnd** project to use SQLite instead.

   > **Note:** This exercise uses SQLite to avoid repetition from the previous exercise. If you prefer to use SQL Server, you can configure a new SQL Server service in Azure, and configure the **BackEnd** project to use this service, by following the procedures in the previous exercise.

    1. Add a reference to the Nuget Package `Microsoft.Data.Sqlite` version `3.1.1` to the **BackEnd** project.
        - **If you're using Visual Studio**, use the **NuGet Package Manager**
    ...
    6. Migrate the database to SQLite:
        ...
        - **If you're using the .NET Core CLI:**
            ...
            6. # TDB COPY DATABASE TO DEPLOYMENT

3. Change the version of the .NET Framework Core for the BackEnd project.
...
3. On the home page, hover over **App Services**, and then click **Create**.
...
4. On the **Web App** page, on the **Basics** tab, spcify the following settings, and then click **Review + create**.
    ...
    | Name | **linuxbackend\<web-app-sufffix\>**, where *\<web-app-suffix\>* is combination of your name and the current date in numeric format
    | Publish | **Code** |
...
1. In the web browser, move to the swagger endpoint of the web API service, **https://linuxbackend\<*web-app-sufffix\>*.azurewebsites.net/swagger**.
...
6. Open the **appsettings.json** file in the root folder of the **FrontEnd** project, and replace the value of the **ServiceUrl** settings with the URL of the **BackEnd** service running as an App Service, **https://linuxbackend\<*web-app-sufffix\>*.azurewebsites.net**.
...
8. Verify that the web application connects successfully to the **BackEnd** web API service, and that you can browse sessions.
