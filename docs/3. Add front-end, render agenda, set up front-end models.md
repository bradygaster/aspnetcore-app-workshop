...
1. In the **FrontEnd** projectO, in the **Pages** folder, open the **Index.cshtml.cs** file. The **Index.cshtml.cs** file contains the **IndexModel** class.

   > **Note:** If you are using Visual Studio, expand the **Index.cshtml** file in the **Solution Explorer** window to see the **Index.cshtml.cs** file.
...
4. Remove the **_logger** field as it is no longer required.
...
6. Modify the **OnGet** page handler method to handle GET requests to the page as shwon below. The code loads the session data dor a specific day of the conference, and identifies the data required to build the day navigation UI:
...
3. Test the solution:
    ...
    - **If you're using Visual Studio 2019:**
        1. In the **Solution Explorer** window, right-click the **ConferencePlanner** solution, and then click **Properties**.
        2. In the **Solution 'ConferencePlanner' Property Pages** dialog box, select **Multiple startup projects**, set the **Action** for the **BackEnd** and **FrontEnd** projects to **Start**, and then click **OK**.
    ...
    - **If you're using the .NET Core CLI:**
        ...
        2. Make a note of the ports in the **applicationUrl** property for the **BackEnd** profile. The profile will look similar to the following JSON content. In this example, the ports are 5000 and 5001:
        ...
        4. Open the **launchSettings.json** file in the **Properties** folder of the **FrontEnd** project.
        5. If the ports for the **applicationUrl** property of the **FrontEnd** profile are the same as those used by the **BackEnd** profile then change them (for example, set them to 6000 and 6001).
        ...
        10. Open a web browser, and navigate to the URL of the **FrontEnd** web application (this will be one of the URLs you specified in the **applicationUrl** property in the **launchSettings.json** file).

4. Verify that the web site displays a list of conference sessions:

    ![The web browser displaying a list of conference sessions](images/3-sessions.png)

5. Stop the web application and web API service:
...
5. Replace the **@foreach** loop that displays the sessions in each timeslot with the following code. This markup renders each session as a formatted card rather than simple text.
...
1. Add a new Razor page to the **FrontEnd** project:
    ...
    - **If you're using Visual Studio 2019**:
        1. In the **Solution Explorer** window, right-click the **Pages** folder in the **FrontEnd** project, click **Add**, and then click **Razor Page**.
...
2. In the **Pages** folder, open the **Session.cshtml.cs** file (expand the **Session.cshtml** file to view this file if you are using Visual Studio 2019).
...
4. Add a code shown below to the **SessionModel** class. As with the **IndexModel** shown earlier, the **SessionModel** will use an **ApiClient** object to send requests to the web API service. The **Session** and **DayOffset** fields will be used to cache information about the sessions retrieved.
...
8. Stop the web application and web API service.
...
3. Add a constructor to the **SpeakerModel** class. The constructor should take an **IApiClient** object and store it in a local field. Also add a **SpeakerResponse** property. You'll use this property to cache information about the speaker. The following snippet shows the code for the class:
...
4. Replace the **OnGet** page handler with an asynchonrous version that retrieves the speaker details and saves them in the model:
...
5. Open the **Speaker.cshtml** file. Replace the code on this page with the following markup. This code displays the details of the speaker, including the sessions presented by the speaker:
    ...
    >```
    >
    > On either oage, when the user clicks the name of a speaker, the **Session** or **Index** page routes a request to the **Speaker** page containing the ID of the speaker.
...
For the final set of tasks in this session, you'll add search functionality to the back-end web API service and the front-end web app. Users will be able to search for speakers and sessions. You'start by adding support for searching to the back-end web API.
...
2. Add a another class file, named  **SearchResult.cs**, to the **ConferenceDTO** project. Add the code shown below to this file. The **SearchResult** class records the sessions or speakers found by the search. The **SearchResultType** enumeration indicates whether the search is for a session or a speaker:
...
3. Switch to the **BackEndEnd** project, and add a new controller, named **SearchController**, to the **Controllers** folder:

    - **If you're using Visual Studio 2019:**
        1. In the **Solution Explorer** window, right-click the **Controllers** folder in the **BackEnd** project, clcik **Add**, and then click **Controller**.
        2. In the **Add New Scaffolded Item** dialog box, select the **API Controller - Empty** template, and then click **Add**
        3. In the **Add an Empty API Controller** dialog box, set the controller name to **SearchController**, and then click **Add*.
...
In this task, you'll modify the **FrontEnd** project to allow the user to submit search requests to the **BackEnd** web API service. You'll add a page that the web app can use to display the results.
...
6. Add a constructor that takes an **IApiClient** object, and save this object in a local field. Also add fields that can store the search term (a string), and the search results:
...
12. On the **Search** enter a string to search for, and then click **Go!**. All matching speakers and session titles will be displayed, as shown in the image below:
...
13. Stop the web application and web API service.

> **Note:** You can find the completed code for the exercises in this session in the Any [Session 3 save point folder](../save-points/3-Front-End-started/).
