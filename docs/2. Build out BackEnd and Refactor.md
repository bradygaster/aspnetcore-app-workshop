...
3. Add a new controller, named **SessionsController**, to the **Controllers** folder in the **BackEnd** project:
    ...
    - **If you're using Visual Studio 2019:**
        ...
        1. Select the **API Controller with actions, using Entity Framework** template.
        2. Set the **Model class** to **Session(BackEnd.Data)**.
        3. Set the **Data context class to **ApplicationDbContext**.
        4. Set the name to **SessionsController**.
...
3. Copy the contents of the [**SessionizeLoader.cs**](../save-points/2-BackEnd-completed/Data/SessionizeLoader.cs) class file in the save point folder into your **SessionizeLoader.cs** file. Note that the **SessionizeLoader** class extends the **DataLoader** abstract class.

4. Edit the **SessionController.cs** file in the **Controllers** folder, and add **Upload** method shown below to the controller. This method takes the name of a JSON file containing conference data, and uses the data loader to read the contents and populate the database.
...
3. If you're using Visual Studio 2019, a web browser window will open automatically. If you are using the .NET Core CLI, open a new web browser window.

4. In the web browser, navigate to the URL **https://localhost:*port*/swagger**. You should see operations available through the **Attendees**, **Sessions**, and **Speakers** controllers.

    ![The **Swagger** UI showing the new controllers and operations.](images/3-controllers.png)

5. In the **Sessions** section, click the **POST** button for for the **api/Sessions/upload** operation, and then click **Try it out**.

6. In the **Parameters** section, at the **file** prompt, click **Browse**.

7. In the **Open** dialog box, for the **File name**, specify the URL **https://github.com/dotnet-presentations/aspnetcore-app-workshop/blob/master/src/BackEnd/Data/Import/NDC_London_2020.json**, and then click **Open**.
...
8. Back in the web browser, click **Execute**, and wait for the data to be uploaded. Verify that the upload is successful (the server response code should be 200)
    ...
    ![The **Swagger** UI. The user has executed the session upload operation.](images/3-upload-sessions.png)
...
10. Verify that the response body contains the details of the conference sessions, in JSON format:

    ![The **Swagger** UI showing the details of sessions.](images/3-sessions.png)

11. Scroll down and click the **GET** button for the **api/Speakers** operation. Click **Try it out**, and then click **Execute**. This time the response body should contain details about the conference speakers an the sessions they are presenting, again in JSON format.
...
**Next**: [Session #3 - Create the FrontEnd web application](3.%20Add%20front-end%2C%20render%20agenda%2C%20set%20up%20front-end%20models.md) | **Previous**: [Session #1 - Create the BackEnd Web API](/docs/1.%20Create%20BackEnd%20API%20project.md)
